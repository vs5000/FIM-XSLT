										...
										<xsl:analyze-string regex="^(S|F|G|D)\d{{11}}$" select="$objID">
											<xsl:matching-substring>
												...
												<xsl:if test="$Meldungen = '1' and fn:substring($objID,1,1) != 'S'">
													<div class="Fehler E1087">Fehler!! E1087: Relation zu einem unzulässigen Objekt.</div>
												</xsl:if>
											</xsl:matching-substring>
											<xsl:non-matching-substring>
												<xsl:if test="$Meldungen = '1'">
		...
		<tr>
			...
			<xsl:choose>
				<xsl:when test="empty($Element/xdf3:hilfetext/text())">
					<td>
						<xsl:if test="$Meldungen = '1'">
							<div class="Warnung W1007">Warnung!! W1007: Der Hilfetext eines Dokumentsteckbriefs sollte befüllt werden.</div>
						</xsl:if>
					</td>
				</xsl:when>
				<xsl:otherwise>
							...
							<xsl:choose>
								...
								<xsl:when test="$minLength != '' or $maxLength != ''">
									<td>
										...
										<xsl:if test="$Meldungen = '1'">
											<div class="Fehler E1015">Fehler!! E1015: Eine Feldlänge darf nur bei einem Datenfeld mit dem Datentyp 'Text' oder 'String.Latin+' angegeben werden.</div>
										</xsl:if>
									</td>
								</xsl:when>
								<xsl:otherwise>
					...
					<td>
						<xsl:if test="$Element/xdf3:codeKey/text() !='' or $Element/xdf3:nameKey/text() !='' or $Element/xdf3:helpKey/text() !=''">
							...
							<xsl:if test="$Meldungen = '1' and not($Element/xdf3:codelisteReferenz)">
								<div class="Fehler E1058">Fehler!! E1058: Spaltendefinitionen dürfen nur angegeben werden, wenn im Datenfeld eine referenzierte Codeliste enthalten ist.</div>
							</xsl:if>
						</xsl:if>
					</td>
				</tr>
						...
						<xsl:otherwise>
							<td>
								...
								<xsl:if test="$Meldungen = '1' and $Element/xdf3:datentyp/code/text() != 'file'">
									<div class="Fehler E1056">Fehler!! E1056: Die Dateigröße darf nur befüllt sein, wenn der Datentyp des Datenfeldes den Wert 'Anlage (Datei)' hat.</div>
								</xsl:if>
							</td>
						</xsl:otherwise>
					</xsl:choose>
					...
					<xsl:choose>
						<xsl:when test="fn:not(empty($Element/xdf3:mediaType[1]/text()))">
							<td>
								...
								<xsl:if test="$Meldungen = '1' and $Element/xdf3:datentyp/code/text() != 'file'">
									<div class="Fehler E1057">Fehler!! E1057: Erlaubte Datentypen dürfen nur angegeben werden, wenn der Datentyp des Datenfeldes den Wert 'Anlage (Datei)' hat.</div>
								</xsl:if>
							</td>
						</xsl:when>
						<xsl:otherwise>
	...
	<xsl:template name="styleandscript">
		...
		<xsl:if test="$JavaScript = '1'">
			...
			<xsl:if test="$MeldungsFazit = '1' and $Meldungen = '1' and $JavaScript = '1'">
				...
				{
					document.getElementById("FehlerListe").insertAdjacentHTML('beforeend', document.querySelectorAll('.E1056').length + " mal E1056: Die Dateigröße darf nur befüllt sein, wenn der Datentyp des Datenfeldes den Wert 'Anlage (Datei)' hat." + "<xsl:text disable-output-escaping="yes">&lt;</xsl:text>br/<xsl:text disable-output-escaping="yes">&gt;</xsl:text>");
				}
				if (document.querySelectorAll('.E1057').length != 0)
				{
					document.getElementById("FehlerListe").insertAdjacentHTML('beforeend', document.querySelectorAll('.E1057').length + " mal E1057: Erlaubte Datentypen dürfen nur angegeben werden, wenn der Datentyp des Datenfeldes den Wert 'Anlage (Datei)' hat." + "<xsl:text disable-output-escaping="yes">&lt;</xsl:text>br/<xsl:text disable-output-escaping="yes">&gt;</xsl:text>");
				}
				if (document.querySelectorAll('.E1058').length != 0)
				{
					document.getElementById("FehlerListe").insertAdjacentHTML('beforeend', document.querySelectorAll('.E1058').length + " mal E1058: Spaltendefinitionen dürfen nur angegeben werden, wenn im Datenfeld eine referenzierte Codeliste enthalten ist." + "<xsl:text disable-output-escaping="yes">&lt;</xsl:text>br/<xsl:text disable-output-escaping="yes">&gt;</xsl:text>");
				}
...
-->
				...
				{
					document.getElementById("FehlerListe").insertAdjacentHTML('beforeend', document.querySelectorAll('.E1087').length + " mal E1087: Relation zu einem unzulässigen Objekt." + "<xsl:text disable-output-escaping="yes">&lt;</xsl:text>br/<xsl:text disable-output-escaping="yes">&gt;</xsl:text>");
				}
