													...
													<xsl:when test="$Element/xdf:feldart/code/text() = 'select'">
														<xsl:choose>
															...
															<xsl:when test="$Element/xdf:datentyp/code/text() = 'int'">
																<td>
																	<xsl:apply-templates select="$Element/xdf:datentyp"/>
																	<xsl:if test="$Meldungen = '1'"><div class="Warnung W1008">Warnung!! W1008: Wenn ein Datenfeld die Feldart 'Auswahl' hat, sollte der Datentyp i. d. R. vom Typ 'Text' sein.</div></xsl:if>
																</td>
															</xsl:when>
															<xsl:otherwise>
																<td>
																		...
																		<xsl:choose>
																			<xsl:when test="$minLength != '' or $maxLength != ''">
																				<td>
																					...
																					<xsl:if test="$Meldungen = '1'"><div class="Fehler E1012">Fehler!! E1012: Bei Datenfeldern mit der Feldart 'Auswahlfeld' oder 'Statisches, read-only Feld' dürfen weder die minimale noch die maximale Feldlänge angegeben werden.</div></xsl:if>
																				</td>
																			</xsl:when>
																			<xsl:otherwise>
																				<td>
																...
																<xsl:choose>
																	<xsl:when test="$minValue != '' or $maxValue != ''">
																		<td>
																			...
																			<xsl:if test="$Meldungen = '1'"><div class="Fehler E1016">Fehler!! E1016: Ein Wertebereich darf nur bei einem Datenfeld mit einem nummerischen Datentyp (Fließkommazahl, Ganzzahl, Währungswert) angegeben werden.</div></xsl:if>
																		</td>
																	</xsl:when>
																	<xsl:otherwise>
																		<td>
																				...
																				<tr>
																					<td>
																						...
																						<xsl:if test="count($Element/xdf:struktur/xdf:enthaelt[xdf:id=$VergleichsElementIDVersion]) &gt; 1">
																							<xsl:if test="$Meldungen = '1'"><div class="Fehler E1004">Fehler!! E1004: In einer Datenfeldgruppe darf ein Baukastenelement nicht mehrfach in derselben Version enthalten sein.</div></xsl:if>
																						</xsl:if>
																					</td>
																					<td>
																						<xsl:value-of select="$ElementVersion"/>
											...
											<tr>
												...
												<xsl:choose>
													<xsl:when test="$VersionsAnzahl &gt; 1">
														<td>
															<xsl:value-of select="$Element/xdf:identifikation/xdf:version"/>
															<xsl:if test="$Meldungen = '1'"><div class="Fehler E1003">Fehler!! E1003: In einem Datenschema darf ein Baukastenelement nicht mehrfach in unterschiedlichen Versionen enthalten sein.</div></xsl:if>
														</td>
													</xsl:when>
													<xsl:otherwise>
														<td>
																...
																<tr>
																	<td>
																		...
																		<xsl:if test="$Tiefe = 1 and count($Element/xdf:struktur/xdf:enthaelt[xdf:id=$VergleichsElementIDVersion]) &gt; 1">
																			<xsl:if test="$Meldungen = '1'"><div class="Fehler E1025">Fehler!! E1025: In einem Datenschema dürfen Baukastenelement auf oberster Ebene nicht mehrfach in derselben Version enthalten sein.</div></xsl:if>
																		</xsl:if>
																	</td>
																	<td>
																		<xsl:value-of select="$ElementVersion"/>
	...
	<xsl:if test="$JavaScript = '1'">
		...
		<script type="text/javascript">
			...
			<xsl:if test="$MeldungsFazit = '1' and $Meldungen = '1' and $JavaScript = '1'">
				...
				{
					document.getElementById("FehlerListe").insertAdjacentHTML('beforeend', document.querySelectorAll('.E1016').length + " mal E1016: Ein Wertebereich darf nur bei einem Datenfeld mit einem nummerischen Datentyp (Fließkommazahl, Ganzzahl, Währungswert) angegeben werden." + "<xsl:text disable-output-escaping="yes">&lt;</xsl:text>br/<xsl:text disable-output-escaping="yes">&gt;</xsl:text>");
				}
