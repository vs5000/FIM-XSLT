									...
									<xsl:when test="./xdf3:codelisteReferenz">
										<xsl:choose>
											<xsl:when test="./xdf3:codelisteReferenz/xdf3:version">
												...
												<xsl:variable name="CodelisteURL" select="fn:concat('https://www.xrepository.de/api/version_codeliste/',./xdf3:codelisteReferenz/xdf3:canonicalVersionUri,'/genericode')"/> 
												
												<xsl:if test="$DebugMode = '4'">
													<xsl:message>
														++++ stammdatenschema / datenfeldgruppe++++
														URL-Codeliste: <xsl:value-of select="$CodelisteURL"/>
													</xsl:message>
												</xsl:if>
												...
												<xsl:variable name="CodelisteInhalt">
													<xsl:choose>
														...
														<xsl:otherwise>
															<xsl:if test="$DebugMode = '4'">
																<xsl:message>                                  LEER: url <xsl:value-of select="$CodelisteURL"/></xsl:message>
															</xsl:if>
														</xsl:otherwise>
													</xsl:choose>
												</xsl:variable>
