...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xdf3="urn:xoev-de:fim:standard:xdatenfelder_3.0.0" xmlns:html="http://www.w3.org/1999/xhtml" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:xdf="urn:xoev-de:fim:standard:xdatenfelder_2" xmlns:gc="http://docs.oasis-open.org/codelist/ns/genericode/1.0/"  xmlns:ext="http://www.xoev.de/de/xrepository/framework/1/extrakte" xmlns:bdt="http://www.xoev.de/de/xrepository/framework/1/basisdatentypen" xmlns:dat="http://www.xoev.de/de/xrepository/framework/1/datenbereitstellung" xmlns:svrl="http://purl.oclc.org/dsdl/svrl" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="html xsl fn xdf3 gc ext bdt dat svrl xdf xs">
	...
	<xsl:variable name="StyleSheetName" select="'XDF2_2_XDF3_0_07_xdf2.xsl'"/> <!-- BackUp, falls fn:static-base-uri() leer -->
	...
	<xsl:variable name="OutputDateiname">
		<xsl:choose>
			<xsl:when test="name(/*) ='xdf:xdatenfelder.stammdatenschema.0102'">
				<xsl:choose>
					<xsl:when test="empty(//xdf:stammdatenschema/xdf:identifikation/xdf:version/text())"><xsl:value-of select="fn:substring($SIDNeu,1,3)"/><xsl:value-of select="$Unternummernkreis"/><xsl:value-of select="fn:substring($SIDNeu,4)"/>_<xsl:value-of select="$NameNeu"/>_xdf3.xml</xsl:when>
					<xsl:otherwise><xsl:value-of select="fn:substring($SIDNeu,1,3)"/><xsl:value-of select="$Unternummernkreis"/><xsl:value-of select="fn:substring($SIDNeu,4)"/>V<xsl:value-of select="//xdf:stammdatenschema/xdf:identifikation/xdf:version"/>.0_<xsl:value-of select="$NameNeu"/>_xdf3.xml</xsl:otherwise>
				</xsl:choose>
			</xsl:when>
	...
	<xsl:template match="xdf:stammdatenschema">
		...
		<xdf3:stammdatenschema>
			...
			<xsl:variable name="Aktualisierungsdatum">
				<xsl:for-each select="./comment()">
					<xsl:for-each select="tokenize(., '\n')">
						<xsl:if test="fn:contains(.,'Aktualisierungsdatum:')">
							<xsl:value-of select="fn:substring-after(.,'Aktualisierungsdatum:')"/>
						</xsl:if>
					</xsl:for-each>
				</xsl:for-each>
			</xsl:variable>
			...
			<xsl:variable name="Dokumentsteckbrief">
				<xsl:for-each select="./comment()">
					<xsl:for-each select="tokenize(., '\n')">
						<xsl:if test="fn:contains(.,'Katalogeintrag:')">
							<xsl:value-of select="fn:substring-after(.,'Katalogeintrag:')"/>
						</xsl:if>
					</xsl:for-each>
				</xsl:for-each>
			</xsl:variable>
			...
			<xsl:variable name="LetzteVersion">
				<xsl:for-each select="./comment()">
					<xsl:for-each select="tokenize(., '\n')">
						<xsl:if test="fn:contains(.,'LetzteVersion:')">
							<xsl:value-of select="fn:substring-after(.,'LetzteVersion:')"/>
						</xsl:if>
					</xsl:for-each>
				</xsl:for-each>
			</xsl:variable>
