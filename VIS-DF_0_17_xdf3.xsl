				...
				<xsl:variable name="Hinweistext">
					...
					<ul>
						...
						<xsl:if test="$ToolAufruf = '1'">
							<li>Klick auf einen &#8658; = �ffnet den hinterlegten Editor mit dem Element</li>
						</xsl:if>
					</ul>
					<div class="Hinweisklein">
						<p><b>Hinweis</b></p>
						<p>Diese Ansicht ist eine m�gliche, limitierte Visualisierung der Umsetzung des Datenschemas. Sie dient dazu die Inhalte des Datenschemas leichter einzusch�tzen und qualit�tszusichern.</p>
						<p>Die gew�hlte Visualisierung hat keinerlei normativen Charakter und stellt somit keinerlei Empfehlung f�r die Umsetzung dar.</p>
						...
						<xsl:when test="name(/*) ='xdf3:xdatenfelder.stammdatenschema.0102'">
							<form name="SDSForm" class="SDSForm" method="post">
								...
								<xsl:if test="$ZeigeDaten = '1'">
									<p><button type="reset" id="zuruecksetzen" class="button">Daten zur�cksetzen</button></p>
								</xsl:if>
			...
			<div class="SDSBezeichnung">
				...
				<xsl:if test="fn:not(empty(/*/xdf3:stammdatenschema/xdf3:hilfetext/text()))">
					...
					<div class="Hilfe" onclick="ZeigeBox(this)">?
						<div class="Popuptext"><xsl:value-of select="/*/xdf3:stammdatenschema/xdf3:hilfetext"/></div>
					</div>
				</xsl:if>
				...
				<xsl:if test="$Infobox = '1'">
					...
					<xsl:variable name="Infoboxinhalt">
						...
						<b>Letzte �nderung:&#160;&#160;</b><xsl:value-of select="fn:format-dateTime(/*/xdf3:stammdatenschema/xdf3:letzteAenderung, '[D01].[M01].[Y0001] [H01]:[m01] Uhr')"/><br/><br/>
					</xsl:variable>
					...
					<xsl:variable name="Infoboxinhalt2">
						<xsl:for-each select="/*/xdf3:stammdatenschema/xdf3:regel">
							...
							<b>Freitextregel:&#160;&#160;</b><xsl:value-of select="./xdf3:freitextRegel"/><br/><br/>
						</xsl:for-each>
					</xsl:variable>
		...
		<div class="SDS">
			<div class="SDSBezeichnung">
				...
				<xsl:if test="fn:not(empty(//xdf3:xdatenfelder.datenfeldgruppe.0103/xdf3:datenfeldgruppe/xdf3:hilfetextEingabe/text()))">
					&#160;&#160;
					<div class="Hilfe" onclick="ZeigeBox(this)">?
						<div class="Popuptext"><xsl:value-of select="//xdf3:xdatenfelder.datenfeldgruppe.0103/xdf3:datenfeldgruppe/xdf3:hilfetextEingabe"/></div>
					</div>
				</xsl:if>
					...
					<xsl:element name="div">
						...
						<xsl:choose>
							...
							<xsl:when test="$minCount != '0' and $maxCount != '1'">
								<button type="button" class="Plus" title="Element hinzuf�gen" onclick="FEDuplizieren(this)">&#43;</button><button type="button" class="Minus" title="Element entfernen" onclick="FEEntfernen(this)" disabled="true">&#8722;</button>&#160;&#160;
								<span class="FEBezeichnung FEMuss FEMehrfach">
									<xsl:value-of select="$Element/xdf3:enthaelt/*/xdf3:bezeichnungEingabe"/>&#160;&#160;
								</span>
							</xsl:when>
										...
										<xsl:when test="$Element/xdf3:enthaelt/*/xdf3:datentyp/code/text() = 'bool'">
											<label>
												<xsl:element name="input">
													...
													<xsl:if test="$Element/xdf3:enthaelt/*/xdf3:inhalt = 'wahr'"><xsl:attribute name="checked">true</xsl:attribute></xsl:if>
													<xsl:if test="$minCount != 0"><xsl:attribute name="required"/></xsl:if>
												</xsl:element>
												ja
											</label>&#160;&#160;&#160;
											<label>
												<xsl:element name="input">
													...
													<xsl:attribute name="type">radio</xsl:attribute>
													<xsl:attribute name="id"><xsl:value-of select="$ElternBaum"/>.<xsl:value-of select="$Element/xdf3:enthaelt/*/xdf3:identifikation/xdf3:id"/>.falsch</xsl:attribute>
													<xsl:attribute name="name"><xsl:value-of select="$ElternBaum"/>.<xsl:value-of select="$Element/xdf3:enthaelt/*/xdf3:identifikation/xdf3:id"/></xsl:attribute>
													<xsl:attribute name="value">falsch</xsl:attribute>
													<xsl:if test="$Element/xdf3:enthaelt/*/xdf3:inhalt = 'falsch'"><xsl:attribute name="checked">true</xsl:attribute></xsl:if>
													<xsl:if test="$minCount != 0"><xsl:attribute name="required"/></xsl:if>
												</xsl:element>
												nein
											</label>
												...
												<div class="Hilfe" onclick="ZeigeBox(this)">!
													<div class="Popuptext">
														<xsl:if test="fn:not(empty($Element/xdf3:enthaelt/*/xdf3:maxSize/text()))">
															Die Datei darf nicht gr��er sein als <xsl:value-of select="number($Element/xdf3:enthaelt/*/xdf3:maxSize) div 1000000"/> MB.<br/><br/>
														</xsl:if>
														<xsl:if test="fn:not(empty($Element/xdf3:enthaelt/*/xdf3:mediaType[1]/text()))">
															Folgende Dateitypen sind zul�ssig:<br/>
									...
									<xsl:element name="select">
										...
										<xsl:choose>
											...
											<xsl:when test="$Element/xdf3:enthaelt/*/xdf3:codelisteReferenz">
												...
												<xsl:variable name="CodelisteURL" select="fn:concat('https://www.xrepository.de/api/version_codeliste/',$Element/xdf3:enthaelt/*/xdf3:codelisteReferenz/xdf3:canonicalVersionUri,'/genericode')"/> 
												
												<xsl:message><xsl:value-of select="$CodelisteURL"/></xsl:message>
												...
												<xsl:choose>
													...
													<xsl:otherwise>
														...
														<xsl:message>NameCodeKey: <xsl:copy-of select="$NameCodeKey"/></xsl:message>
														<xsl:message>NameCodenameKey: <xsl:copy-of select="$NameCodenameKey"/></xsl:message>
								...
								<div class="Hilfe" onclick="ZeigeBox(this)">?
									<div class="Popuptext">
										<xsl:if test="fn:not(empty($Element/xdf3:enthaelt/*/xdf3:hilfetextEingabe/text()))">
											<xsl:value-of select="$Element/xdf3:enthaelt/*/xdf3:hilfetextEingabe"/>
										</xsl:if>
								...
								<xsl:variable name="Infoboxinhalt">
									...
									<xsl:choose>
										<xsl:when test="$Element/xdf3:enthaelt/*/xdf3:feldart = 'input'">
											<xsl:choose>
												<xsl:when test="$Element/xdf3:enthaelt/*/xdf3:datentyp/code/text() = 'text' or $Element/xdf3:enthaelt/*/xdf3:datentyp/code/text() = 'text_latin'">
													<xsl:if test="$minLength != ''"><b>Minimall�nge:</b>&#160;&#160;<xsl:value-of select="$minLength"/><br/></xsl:if>
													<xsl:if test="$maxLength != ''"><b>Maximall�nge:</b>&#160;&#160;<xsl:value-of select="$maxLength"/><br/></xsl:if>
			...
			<xsl:otherwise>
				<xsl:element name="div">
					...
					<xsl:choose>
						<xsl:when test="$minCount = $maxCount and $maxCount != '1'">
							...
							<span>
								<xsl:if test="fn:not(empty($Element/xdf3:enthaelt/*/xdf3:hilfetextEingabe/text()))">
									&#160;&#160;
									<div class="Hilfe" onclick="ZeigeBox(this)">?
										<div class="Popuptext"><xsl:value-of select="$Element/xdf3:enthaelt/*/xdf3:hilfetextEingabe"/></div>
									</div>
								</xsl:if>
								<xsl:if test="$Infobox = '1'">
									&#160;&#160;
						...
						<xsl:when test="$minCount != '0' and $maxCount != '1'">
							<button type="button" class="Plus" title="Element hinzuf�gen" onclick="FGDuplizieren(this)">&#43;</button><button type="button" class="Minus" title="Element entfernen" onclick="FGEntfernen(this)" disabled="true">&#8722;</button>
