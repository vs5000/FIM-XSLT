					...
					<xsl:choose>
						<xsl:when test="$VersionsAnzahl &gt; 1">
							<td>
								<xsl:value-of select="$Element/xdf3:identifikation/xdf3:version"/>
								<xsl:if test="$Meldungen = '1'">
									<div class="Fehler E1003">Fehler!! E1003: Im selben Datenschema darf ein Baukastenelement nicht in unterschiedlichen Versionen verwendet werden.</div>
								</xsl:if>
							</td>
						</xsl:when>
						<xsl:otherwise>
							...
							<xsl:choose>
								...
								<xsl:when test="$Element/xdf3:datentyp/code/text() = 'int'">
									<td>
										<xsl:apply-templates select="$Element/xdf3:datentyp"/>
										<xsl:if test="$Meldungen = '1'">
											<div class="Warnung W1008">Warnung!! W1008: Bei Auswahlfeldern ist der Datentyp i. d. R. vom Typ 'Text'.</div>
										</xsl:if>
									</td>
								</xsl:when>
								<xsl:otherwise>
											...
											<xsl:choose>
												<xsl:when test="$minLength != '' or $maxLength != ''">
													<td>
														...
														<xsl:if test="$Meldungen = '1'">
															<div class="Fehler E1012">Fehler!! E1012: Bei Datenfeldern mit der Feldart 'Auswahlfeld' oder 'Statisches, read-only Feld' sollten weder die minimale noch die maximale Feldlänge angegeben werden.</div>
														</xsl:if>
													</td>
												</xsl:when>
												<xsl:otherwise>
				...
				<tr>
					<xsl:choose>
						...
						<xsl:when test="$Element/xdf3:feldart/code/text() = 'select' and not($Element/xdf3:codelisteReferenz or $Element/xdf3:werte)">
							<td>
								<xsl:if test="$Meldungen = '1'">
									<div class="Fehler E1018">Fehler!! E1018: Wenn ein Datenfeld die Feldart 'Auswahlfeld' hat, muss eine Codeliste oder Werteliste zugeordnet sein.</div>
								</xsl:if>
							</td>
						</xsl:when>
													...
													<xsl:choose>
														<xsl:when test="$VergleichsVersion = ''">
															<xsl:if test="count($Element/xdf3:struktur/./xdf3:enthaelt/*/xdf3:identifikation[xdf3:id=$VergleichsElement and not(xdf3:version)]) &gt; 1">
																<xsl:if test="$Meldungen = '1'">
																	<div class="Fehler E1004">Fehler!! E1004: In einer Datenfeldgruppe darf kein Baukastenelement mehrfach in derselben Version enthalten sein.</div>
																</xsl:if>
															</xsl:if>
														</xsl:when>
														<xsl:otherwise>
			...
			<td>
				...
				<xsl:choose>
					...
					<xsl:when test="substring($Element/xdf3:canonicalIdentification/text(),1,4) != 'urn:'">
						<xsl:if test="$Meldungen = '1'">
							<div class="Fehler E1005">Fehler!! E1005: Die Kennung ist nicht als URN formuliert, z.B. urn:de:fim:codeliste:dokumenttyp.</div>
						</xsl:if>
					</xsl:when>
					<xsl:otherwise>
					...
					<xsl:choose>
						<xsl:when test="$VergleichsVersion = ''">
							<xsl:if test="$Tiefe = 1 and count($Element/xdf3:struktur/./xdf3:enthaelt/*/xdf3:identifikation[xdf3:id=$VergleichsElement and not(xdf3:version)]) &gt; 1">
								<xsl:if test="$Meldungen = '1'">
									<div class="Fehler E1025">Fehler!! E1025: In einem Datenschema darf auf oberster Ebene kein Baukastenelement mehrfach in derselben Version enthalten sein.</div>
								</xsl:if>
							</xsl:if>
						</xsl:when>
						<xsl:otherwise>
