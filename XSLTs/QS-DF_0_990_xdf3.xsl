...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:html="http://www.w3.org/1999/xhtml" xmlns="http://www.w3.org/1999/xhtml" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:xdf3="urn:xoev-de:fim:standard:xdatenfelder_3.0.0" xmlns:gc="http://docs.oasis-open.org/codelist/ns/genericode/1.0/" xmlns:ext="http://www.xoev.de/de/xrepository/framework/1/extrakte" xmlns:bdt="http://www.xoev.de/de/xrepository/framework/1/basisdatentypen" xmlns:dat="http://www.xoev.de/de/xrepository/framework/1/datenbereitstellung" xmlns:svrl="http://purl.oclc.org/dsdl/svrl" exclude-result-prefixes="html">
	...
	<xsl:variable name="StyleSheetName" select="'QS-DF_0_990_xdf3.xsl'"/> <!-- BackUp, falls fn:static-base-uri() leer -->
			...
			<xsl:when test="$DateiOutput ='1'">
				<xsl:variable name="OutputDateiname">
					<xsl:choose>
						...
						<xsl:when test="name(/*) ='xdf3:xdatenfelder.datenfeldgruppe.0103'">Bericht_<xsl:value-of select="/*/xdf3:datenfeldgruppe/xdf3:identifikation/xdf3:id"/>
							<xsl:if test="/*/xdf3:datenfeldgruppe/xdf3:identifikation/xdf3:version">V<xsl:value-of select="/*/xdf3:datenfeldgruppe/xdf3:identifikation/xdf3:version"/>
							</xsl:if>.html</xsl:when>
						<xsl:when test="name(/*) ='xdf3:xdatenfelder.dokumentsteckbrief.0101'">Bericht_<xsl:value-of select="/*/xdf3:dokumentsteckbrief/xdf3:identifikation/xdf3:id"/>
							<xsl:if test="/*/xdf3:dokumentsteckbrief/xdf3:identifikation/xdf3:version">V<xsl:value-of select="/*/xdf3:dokumentsteckbrief/xdf3:identifikation/xdf3:version"/>
							</xsl:if>.html</xsl:when>
					...
					<xsl:choose>
						...
						<xsl:when test="name(/*) ='xdf3:xdatenfelder.datenfeldgruppe.0103'">
							<h1>
								<xsl:choose>
									<xsl:when test="$Meldungen = '1'">
											Qualitätsbericht Datenfeldgruppe <xsl:value-of select="/*/xdf3:datenfeldgruppe/xdf3:identifikation/xdf3:id"/>
										<xsl:if test="/*/xdf3:datenfeldgruppe/xdf3:identifikation/xdf3:version">&#160;Version&#160;<xsl:value-of select="/*/xdf3:datenfeldgruppe/xdf3:identifikation/xdf3:version"/>
										</xsl:if>
									</xsl:when>
									<xsl:otherwise>
											Übersicht Datenfeldgruppe <xsl:value-of select="/*/xdf3:datenfeldgruppe/xdf3:identifikation/xdf3:id"/>
										<xsl:if test="/*/xdf3:datenfeldgruppe/xdf3:identifikation/xdf3:version">&#160;Version&#160;<xsl:value-of select="/*/xdf3:datenfeldgruppe/xdf3:identifikation/xdf3:version"/>
										</xsl:if>
									</xsl:otherwise>
								</xsl:choose>
		...
		<tr>
			...
			<xsl:choose>
				<xsl:when test="empty($Element/xdf3:bezeichnung/text())">
					<td>
						<xsl:if test="$Meldungen = '1'">
							<div class="Fehler E1050">Fehler!! E1050: Die Bezeichnung muss befüllt sein.</div>
						</xsl:if>
					</td>
				</xsl:when>
				<xsl:otherwise>
...
-->
	...
	<xsl:template name="dokumentsteckbriefdetails">
		...
		<tr>
			...
			<xsl:choose>
				<xsl:when test="empty($Element/xdf3:definition/text())">
					<td>
						<xsl:if test="$Meldungen = '1'">
							<div class="Fehler E1049">Fehler!! E1049: Die Definition eines Dokumentsteckbriefs muss befüllt sein.</div>
						</xsl:if>
					</td>
				</xsl:when>
				<xsl:otherwise>
		...
		<xsl:if test="($Element/xdf3:istAbstrakt = 'true' and $Meldungen = '1') or count($Element/xdf3:relation[xdf3:praedikat/code = 'VKN'])">
			<tr>
				<td>
					<b>Zugeordnete<br/>Dokumentsteckbriefe</b> <!-- TODO Fehlermeldungen -->
				</td>
				<td>
					<table>
						<thead>
							...
							<xsl:if test="$Meldungen = '1'">
								<xsl:choose>
									<xsl:when test="$Element/xdf3:istAbstrakt = 'false'">
										<tr>
											<td>
												<div class="Fehler E1053">Fehler!! E1053: Zu einem konkreten Dokumentsteckbrief dürfen keine Dokumentsteckbriefe zugeordnet werden.</div>
											</td>
										</tr>
									</xsl:when>
									<xsl:when test="$Element/xdf3:istAbstrakt = 'true' and count($Element/xdf3:relation[xdf3:praedikat/code = 'VKN']) &lt; 2">
										<tr>
											<td>
												<div class="Fehler E1054">Fehler!! E1054: Zu einem abstrakten Dokumentsteckbrief müssen mindestens zwei Dokumentsteckbriefe zugeordnet werden.</div>
											</td>
										</tr>
									</xsl:when>
								</xsl:choose>
		...
		<tr>
			...
			<xsl:choose>
				<xsl:when test="empty($Element/xdf3:hilfetext/text())">
					<td>
						<xsl:if test="$Meldungen = '1'">
							<div class="Fehler W1007">Fehler!! W1007: Der Hilfetext eines Dokumentsteckbriefs sollte befüllt werden.</div>
						</xsl:if>
					</td>
				</xsl:when>
				<xsl:otherwise>
...
-->
	...
	<xsl:template name="codelistedetails">
		...
		<tr>
			<xsl:variable name="CodelisteURN"><xsl:value-of select="$Element/xdf3:canonicalIdentification"/><xsl:if test="$Element/xdf3:version">_<xsl:value-of select="$Element/xdf3:version"/></xsl:if></xsl:variable>
			<xsl:variable name="XMLCodelisteURL"><xsl:choose><xsl:when test="fn:empty($Element/xdf3:version/text())"><xsl:value-of select="fn:concat($XMLXRepoOhneVersionPfadPrefix,$CodelisteURN,$XMLXRepoOhneVersionPfadPostfix)"/></xsl:when><xsl:otherwise><xsl:value-of select="fn:concat($XMLXRepoMitVersionPfadPrefix,$CodelisteURN,$XMLXRepoMitVersionPfadPostfix)"/></xsl:otherwise></xsl:choose></xsl:variable>
			<xsl:variable name="DocCodelisteURL"><xsl:choose><xsl:when test="fn:empty($Element/xdf3:version/text())"><xsl:value-of select="fn:concat($DocXRepoOhneVersionPfadPrefix,$CodelisteURN,$DocXRepoOhneVersionPfadPostfix)"/></xsl:when><xsl:otherwise><xsl:value-of select="fn:concat($DocXRepoMitVersionPfadPrefix,$CodelisteURN,$DocXRepoMitVersionPfadPostfix)"/></xsl:otherwise></xsl:choose></xsl:variable>
					...
					<xsl:analyze-string regex="^urn:[A-Za-z0-9][A-Za-z0-9-]{{0,31}}:([A-Za-z0-9()+,\-.:=@;\$_!*&apos;]|%[0-9A-Fa-f]{{2}})+$" select="$Element/xdf3:canonicalIdentification">
						<xsl:matching-substring>
							...
							<xsl:if test="$Meldungen = '1' and $CodelisteInhalt = ''">
								<div class="Warnung W1017">Warnung!! W1017: Auf die Codeliste kann nicht im XRepository zugegriffen werden. Dies könnte auf einen Fehler in der URN oder der Version der Codeliste zurückzuführen sein.</div>
							</xsl:if>
						</xsl:matching-substring>
						<xsl:non-matching-substring>
							<xsl:if test="$Meldungen = '1'">
	...
	<xsl:template name="styleandscript">
		...
		<xsl:if test="$JavaScript = '1'">
			...
			<xsl:if test="$MeldungsFazit = '1' and $Meldungen = '1' and $JavaScript = '1'">
				...
				{
					document.getElementById("FehlerListe").insertAdjacentHTML('beforeend', document.querySelectorAll('.E1047').length + " mal E1045: E1047: In der Multiplizität muss die minimale Anzahl kleiner oder gleich der maximalen Anzahl sein." + "<xsl:text disable-output-escaping="yes">&lt;</xsl:text>br/<xsl:text disable-output-escaping="yes">&gt;</xsl:text>");
				}
				...
				{
					document.getElementById("FehlerListe").insertAdjacentHTML('beforeend', document.querySelectorAll('.E1049').length + " mal E1049: Die Definition eines Dokumentsteckbriefs muss befüllt sein." + "<xsl:text disable-output-escaping="yes">&lt;</xsl:text>br/<xsl:text disable-output-escaping="yes">&gt;</xsl:text>");
				}
				if (document.querySelectorAll('.E1050').length != 0)
				{
					document.getElementById("FehlerListe").insertAdjacentHTML('beforeend', document.querySelectorAll('.E1050').length + " mal E1050: Die Bezeichnung muss befüllt sein." + "<xsl:text disable-output-escaping="yes">&lt;</xsl:text>br/<xsl:text disable-output-escaping="yes">&gt;</xsl:text>");
				}
				...
				{
					document.getElementById("FehlerListe").insertAdjacentHTML('beforeend', document.querySelectorAll('.E1053').length + " mal E1053: Zu einem konkreten Dokumentsteckbrief dürfen keine Dokumentsteckbriefe zugeordnet werden." + "<xsl:text disable-output-escaping="yes">&lt;</xsl:text>br/<xsl:text disable-output-escaping="yes">&gt;</xsl:text>");
				}
				if (document.querySelectorAll('.E1054').length != 0)
				{
					document.getElementById("FehlerListe").insertAdjacentHTML('beforeend', document.querySelectorAll('.E1054').length + " mal E1054: Zu einem abstrakten Dokumentsteckbrief müssen mindestens zwei Dokumentsteckbriefe zugeordnet werden." + "<xsl:text disable-output-escaping="yes">&lt;</xsl:text>br/<xsl:text disable-output-escaping="yes">&gt;</xsl:text>");
				}
