...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xdf3="urn:xoev-de:fim:standard:xdatenfelder_3.0.0" xmlns:html="http://www.w3.org/1999/xhtml" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:xdf="urn:xoev-de:fim:standard:xdatenfelder_2" xmlns:gc="http://docs.oasis-open.org/codelist/ns/genericode/1.0/"  xmlns:ext="http://www.xoev.de/de/xrepository/framework/1/extrakte" xmlns:bdt="http://www.xoev.de/de/xrepository/framework/1/basisdatentypen" xmlns:dat="http://www.xoev.de/de/xrepository/framework/1/datenbereitstellung" xmlns:svrl="http://purl.oclc.org/dsdl/svrl" exclude-result-prefixes="html xsl fn xdf3 gc ext bdt dat svrl xdf">
	...
	<xsl:variable name="StyleSheetName" select="'XDF2_2_XDF3_0_05_xdf2.xsl'"/> <!-- BackUp, falls fn:static-base-uri() leer -->
	...
	<xsl:variable name="OutputDateiname">
		<xsl:choose>
			<xsl:when test="name(/*) ='xdf:xdatenfelder.stammdatenschema.0102'">
				<xsl:choose>
					<xsl:when test="empty(//xdf:stammdatenschema/xdf:identifikation/xdf:version/text())"><xsl:value-of select="fn:substring($SIDNeu,1,3)"/><xsl:value-of select="$Unternummernkreis"/><xsl:value-of select="fn:substring($SIDNeu,4)"/>_xdf3.xml</xsl:when>
					<xsl:otherwise><xsl:value-of select="fn:substring($SIDNeu,1,3)"/><xsl:value-of select="$Unternummernkreis"/><xsl:value-of select="fn:substring($SIDNeu,4)"/>V<xsl:value-of select="//xdf:stammdatenschema/xdf:identifikation/xdf:version"/>.0_xdf3.xml</xsl:otherwise>
				</xsl:choose>
			</xsl:when>
	...
	<xsl:template match="xdf:stammdatenschema">
		...
		<xdf3:stammdatenschema>
			...
			<xsl:variable name="BeschreibungNeu">
				...
				ACHTUNG: ein Stammdatenschema muss Unterelemente in XDF3 haben, deshalb wurde das BOB-Feld F60000000330 - Datum (aktuelles) eingefügt, um Schemakonformität sicherzustellen.</xsl:if></xsl:variable>
			...
			<xsl:otherwise>
					<xdf3:struktur>
						...
						<xdf3:enthaelt>
							<xdf3:datenfeld>
								<xdf3:identifikation>
									<xdf3:id>F60000000330</xdf3:id>
									<xdf3:version>1.2.0</xdf3:version>
								</xdf3:identifikation>
								<xdf3:name>Datum (aktuelles)</xdf3:name>
								<xdf3:beschreibung>Falls technisch möglich, kann dieses Feld auch automatisch mit dem aktuellen Datum belegt werden. In diesem Fall sollte das Feld als read-only gestaltet werden.</xdf3:beschreibung>
								<xdf3:definition>Dem Kalender entsprechende Zeitangabe.</xdf3:definition>
								<xdf3:freigabestatus listURI="urn:xoev-de:xprozess:codeliste:status" listVersionID="2020-03-19">
									<code>6</code>
								</xdf3:freigabestatus>
								<xdf3:statusGesetztAm>2021-03-31</xdf3:statusGesetztAm>
								...
								<xdf3:veroeffentlichungsdatum>2021-03-31</xdf3:veroeffentlichungsdatum>
								<xdf3:letzteAenderung>2023-08-01T13:17:59.226717Z</xdf3:letzteAenderung>
								<xdf3:bezeichnungEingabe>Datum</xdf3:bezeichnungEingabe>
								<xdf3:bezeichnungAusgabe>Datum</xdf3:bezeichnungAusgabe>
								...
								<xdf3:hilfetextEingabe>Geben Sie das aktuelle Datum an.</xdf3:hilfetextEingabe>
								<xdf3:hilfetextAusgabe>Das aktuelle Datum</xdf3:hilfetextAusgabe>
	...
	<xsl:template match="xdf:datenfeldgruppe">
		...
		<xdf3:datenfeldgruppe>
			...
			<xsl:variable name="BeschreibungNeu">
				...
				Version: <xsl:value-of select="./xdf:identifikation/xdf:version"/></xsl:if><xsl:if test="not(empty(./xdf:status/code/text()))">
				Status: <xsl:value-of select="./xdf:status/code"/></xsl:if><xsl:if test="fn:count(./xdf:struktur) = 0">
				ACHTUNG: eine Datenfeldgruppe muss Unterelemente in XDF3 haben, deshalb wurde das BOB-Feld F60000000330 - Datum (aktuelles) eingefügt, um Schemakonformität sicherzustellen.</xsl:if></xsl:variable>
	...
	<xsl:template name="codelistenInhalte">
		...
		<xsl:if test="$InterneCodeliste!='' or $ExterneCodelisteErreichbar!=''">
			...
			<xsl:if test="fn:string-length($CodelisteInhalt) &gt; 9">
				<xsl:if test="not(empty($CodelisteInhalt/*/gc:Identification/gc:ShortName/text())) or not(empty($CodelisteInhalt/*/Identification/ShortName/text()))">Kurzname: <xsl:value-of select="$CodelisteInhalt/*/gc:Identification/gc:ShortName"/><xsl:value-of select="$CodelisteInhalt/*/Identification/ShortName"/></xsl:if><xsl:if test="not(empty($CodelisteInhalt/*/gc:Identification/gc:LongName/text())) or not(empty($CodelisteInhalt/*/Identification/LongName/text()))"><xsl:text>
