...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:html="http://www.w3.org/1999/xhtml" xmlns="http://www.w3.org/1999/xhtml" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:xdf="urn:xoev-de:fim:standard:xdatenfelder_2" xmlns:gc="http://docs.oasis-open.org/codelist/ns/genericode/1.0/" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="html xs">
	...
	<xsl:variable name="StyleSheetName" select="'QS-DF_0_970_xdf2.xsl'"/> <!-- BackUp, falls fn:static-base-uri() leer -->
			...
			<xsl:when test="$DateiOutput ='1'">
				<xsl:variable name="OutputDateiname">
					<xsl:choose>
						...
						<xsl:when test="name(/*) ='xdf:fim.backup'">Bericht_<xsl:value-of select="fn:substring-before($InputDateiname,'.xml')"/>.html</xsl:when>
		...
		<tr>
			...
			<xsl:choose>
				<xsl:when test="empty($Element/xdf:bezeichnungEingabe/text())">
					<td>
						<xsl:if test="$Meldungen = '1'">
							<div class="Fehler E1009">Fehler!! E1009: Die Bezeichnung Eingabe muss befüllt sein.</div>
						</xsl:if>
					</td>
				</xsl:when>
				<xsl:otherwise>
											...
											<th>
												...
												<xsl:if test="$Meldungen = '1' and ./@Id = $CodeKeySpalte">
													<xsl:if test="count(distinct-values($CodelisteInhalt/*/gc:SimpleCodeList/gc:Row/gc:Value[@ColumnRef=$CodeKeySpalte])) &lt; count($CodelisteInhalt/*/gc:SimpleCodeList/gc:Row/gc:Value[@ColumnRef=$CodeKeySpalte])">
														<div class="Fehler E1045">Fehler!! E1045: Innerhalb einer Codeliste dürfen Codes (Schlüssel) nicht doppelt verwendet werden.</div>
													</xsl:if>
												</xsl:if>
											</th>
										</xsl:for-each>
...
<!-- nur XDF2 -->
				...
				{
					document.getElementById("FehlerListe").insertAdjacentHTML('beforeend', document.querySelectorAll('.E1045').length + " mal E1045: Innerhalb einer Codeliste dürfen Codes (Schlüssel) nicht doppelt verwendet werden." + "<xsl:text disable-output-escaping="yes">&lt;</xsl:text>br/<xsl:text disable-output-escaping="yes">&gt;</xsl:text>");
				}
				if (document.querySelectorAll('.E1050').length != 0)
				{
					document.getElementById("FehlerListe").insertAdjacentHTML('beforeend', document.querySelectorAll('.E1050').length + " mal E1050: Die Bezeichnung Eingabe muss befüllt sein." + "<xsl:text disable-output-escaping="yes">&lt;</xsl:text>br/<xsl:text disable-output-escaping="yes">&gt;</xsl:text>");
				}
