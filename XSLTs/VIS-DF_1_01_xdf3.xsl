...
<xsl:stylesheet 
	...
	<xsl:variable name="StyleSheetName" select="'VIS-DF_1_01_xdf3.xsl'"/> <!-- BackUp, falls fn:static-base-uri() leer -->
						...
						<xsl:variable name="CodelisteInhalt">
							<xsl:choose>
								...
								<xsl:when test="$CodelistenInhalt = '0'">
									<xsl:if test="$DebugMode = '4'">
										<xsl:message>                                  NICHT LADEN: <xsl:value-of select="$CodelisteURL"/></xsl:message>
									</xsl:if>
								</xsl:when>
