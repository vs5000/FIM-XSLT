...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xdf3="urn:xoev-de:fim:standard:xdatenfelder_3.0.0" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:gc="http://docs.oasis-open.org/codelist/ns/genericode/1.0/" xmlns:dat="http://www.xoev.de/de/xrepository/framework/1/datenbereitstellung" exclude-result-prefixes="xsl fn xdf3 gc dat">
	...
	<xsl:variable name="StyleSheetName" select="'XDF3_2_XFall_1_01_xdf3.xsl'"/>
	...
	<xsl:variable name="OutputDateiname">
		<xsl:value-of select="fn:substring-before($InputDateiname,'.xml')"/>_xfall.xsd</xsl:variable>
				...
				<xsl:choose>
					<xsl:when test="$MitVersionen = '1'">
						<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns="urn:fim-de:xfall:datenschema:xdf3" targetNamespace="urn:fim-de:xfall:datenschema:xdf3" xmlns:vc="http://www.w3.org/2007/XMLSchema-versioning" elementFormDefault="qualified" attributeFormDefault="unqualified">
							<xsl:comment>
								<xsl:text>           </xsl:text>Erstellt am: <xsl:value-of select="format-date(fn:current-date(),'[D01].[M01].[Y0001]')"/> um <xsl:value-of select="format-time(fn:current-time(),'[H01]:[m01]')"/> Uhr<xsl:text>           </xsl:text>XSLT: <xsl:choose>
								...
								<xsl:if test="not(empty($DocumentURI))">
									<xsl:text>           </xsl:text>XML: <xsl:call-template name="fileName">
					...
					<xsl:otherwise>
						<xsl:variable name="NSDSID">urn:fim-de:xfall:xdf3:<xsl:value-of select="./*/xdf3:stammdatenschema/xdf3:identifikation/xdf3:id"/>
						...
						<xsl:element name="schema" namespace="http://www.w3.org/2001/XMLSchema">
							...
							<xsl:namespace name="xdf" select="$NSDSID"/>
							<xsl:attribute name="targetNamespace"><xsl:value-of select="$NSDSID"/></xsl:attribute>
	...
	<xsl:template match="xdf3:stammdatenschema">
		...
		<xs:element name="{$ElementName}">
			<xsl:if test="$Annotationen = '1'">
				<xs:annotation>
					<xs:documentation>
						Datenschema: <xsl:value-of select="./xdf3:name"/>
					</xs:documentation>
				</xs:annotation>
			</xsl:if>
			<xs:complexType>
	...
	<xsl:template match="xdf3:datenfeldgruppe">
		...
		<xsl:variable name="Min">
			<xsl:choose>
				<xsl:when test="./xdf3:feldart/code/text() = 'label'">0</xsl:when>
				<xsl:otherwise>
		...
		<xs:element name="{$ElementName}">
			...
			<xsl:if test="$Annotationen = '1'">
				<xs:annotation>
					<xs:documentation>Datenfeldgruppe: <xsl:value-of select="./xdf3:name"/>
						<xsl:if test="$MitVersionen = '0' and fn:string(./xdf3:identifikation/xdf3:version)"> - Version: <xsl:value-of select="./xdf3:identifikation/xdf3:version"/>
						</xsl:if>
					</xs:documentation>
				</xs:annotation>
			</xsl:if>
		...
		<xs:element name="{$ElementName}">
			...
			<xsl:choose>
				<xsl:when test="./xdf3:feldart/code/text() = 'label'">
					...
					<xsl:if test="$Annotationen = '1'">
						<xs:annotation>
							<xs:documentation>Datenfeld: <xsl:value-of select="./xdf3:name"/> - <xsl:if test="$MitVersionen = '0' and fn:string(./xdf3:identifikation/xdf3:version)">Version: <xsl:value-of select="./xdf3:identifikation/xdf3:version"/> - </xsl:if>Typ: Hinweistext</xs:documentation>
						</xs:annotation>
					</xsl:if>
				</xsl:when>
				<xsl:when test="./xdf3:feldart/code/text() = 'select'">
					<xsl:choose>
						<xsl:when test="./xdf3:werte">
							<xsl:choose>
								<xsl:when test="$Praezisierungen = '1'">
									<xsl:if test="$Annotationen = '1'">
										<xs:annotation>
											<xs:documentation>Datenfeld: <xsl:value-of select="./xdf3:name"/> - <xsl:if test="$MitVersionen = '0' and fn:string(./xdf3:identifikation/xdf3:version)">Version: <xsl:value-of select="./xdf3:identifikation/xdf3:version"/> - </xsl:if>Typ: Auswahlliste (Werteliste)</xs:documentation>
										</xs:annotation>
									</xsl:if>
									<xs:simpleType>
										<xs:restriction base="xs:token">
											<xsl:for-each select="./xdf3:werte/xdf3:wert">
												<xs:enumeration value="{./xdf3:code}">
													<xsl:if test="$Annotationen = '1'">
														<xs:annotation>
															<xs:documentation>Auswahlwert: <xsl:value-of select="./xdf3:name"/>
															</xs:documentation>
														</xs:annotation>
													</xsl:if>
												</xs:enumeration>
							...
							<xsl:choose>
								<xsl:when test="$Praezisierungen = '1' and $CodelistenInhalt ='1'">
									<xsl:variable name="RichtigeVersion">
										<xsl:choose>
											<xsl:when test="not(fn:string(./xdf3:codelisteReferenz/xdf3:version))">
											...
											<xsl:otherwise>
												<xsl:value-of select="./xdf3:codelisteReferenz/xdf3:version"/>
											</xsl:otherwise>
										</xsl:choose>
									</xsl:variable>
									...
									<xsl:choose>
											...
											<xsl:if test="$Annotationen = '1'">
												<xsl:choose>
													<xsl:when test="not(fn:string(./xdf3:codelisteReferenz/xdf3:version)) and $AktuelleCodelisteLaden ='0'">
														<xs:annotation>
															<xs:documentation>Datenfeld: <xsl:value-of select="./xdf3:name"/> - <xsl:if test="$MitVersionen = '0' and fn:string(./xdf3:identifikation/xdf3:version)">Version: <xsl:value-of select="./xdf3:identifikation/xdf3:version"/> - </xsl:if>Typ: Auswahlliste (Externe Codeliste: <xsl:value-of select="./xdf3:codelisteReferenz/xdf3:canonicalIdentification"/> ohne Versionsangabe)</xs:documentation>
														</xs:annotation>
													</xsl:when>
													<xsl:when test="not(fn:string(./xdf3:codelisteReferenz/xdf3:version)) and $RichtigeVersion/text() != 'unbestimmt'">
														<xs:annotation>
															<xs:documentation>Datenfeld: <xsl:value-of select="./xdf3:name"/> - <xsl:if test="$MitVersionen = '0' and fn:string(./xdf3:identifikation/xdf3:version)">Version: <xsl:value-of select="./xdf3:identifikation/xdf3:version"/> - </xsl:if>Typ: Auswahlliste (Externe Codeliste: <xsl:value-of select="./xdf3:codelisteReferenz/xdf3:canonicalIdentification"/> in der aktuellen Version <xsl:value-of select="$RichtigeVersion"/> konnte nicht geladen werden)</xs:documentation>
														</xs:annotation>
													</xsl:when>
													<xsl:when test="$RichtigeVersion/text() = 'unbestimmt'">
														<xs:annotation>
															<xs:documentation>Datenfeld: <xsl:value-of select="./xdf3:name"/> - <xsl:if test="$MitVersionen = '0' and fn:string(./xdf3:identifikation/xdf3:version)">Version: <xsl:value-of select="./xdf3:identifikation/xdf3:version"/> - </xsl:if>Typ: Auswahlliste (Externe Codeliste: <xsl:value-of select="./xdf3:codelisteReferenz/xdf3:canonicalIdentification"/> konnte nicht geladen werden)</xs:documentation>
														</xs:annotation>
													</xsl:when>
													<xsl:otherwise>
														<xs:annotation>
										...
										<xsl:otherwise>
											<xsl:if test="$Annotationen = '1'">
												<xsl:choose>
													<xsl:when test="fn:string(./xdf3:codelisteReferenz/xdf3:version)">
														<xs:annotation>
															<xs:documentation>Datenfeld: <xsl:value-of select="./xdf3:name"/> - <xsl:if test="$MitVersionen = '0' and fn:string(./xdf3:identifikation/xdf3:version)">Version: <xsl:value-of select="./xdf3:identifikation/xdf3:version"/> - </xsl:if>Typ: Auswahlliste (Externe Codeliste: <xsl:value-of select="./xdf3:codelisteReferenz/xdf3:canonicalIdentification"/>_<xsl:value-of select="./xdf3:codelisteReferenz/xdf3:version"/>)</xs:documentation>
														</xs:annotation>
													</xsl:when>
													<xsl:otherwise>
														<xs:annotation>
															<xs:documentation>Datenfeld: <xsl:value-of select="./xdf3:name"/> - <xsl:if test="$MitVersionen = '0' and fn:string(./xdf3:identifikation/xdf3:version)">Version: <xsl:value-of select="./xdf3:identifikation/xdf3:version"/> - </xsl:if>Typ: Auswahlliste (Externe Codeliste: <xsl:value-of select="./xdf3:codelisteReferenz/xdf3:canonicalIdentification"/> in der aktuellen Version <xsl:value-of select="$RichtigeVersion"/>)</xs:documentation>
														</xs:annotation>
													</xsl:otherwise>
												</xsl:choose>
											</xsl:if>
											<xsl:variable name="NameCodeKey">
												<xsl:choose>
													<xsl:when test="fn:string(./xdf3:codeKey)">
														<xsl:value-of select="./xdf3:codeKey"/>
													</xsl:when>
													<xsl:otherwise>
											...
											<xsl:variable name="NameCodenameKey">
												<xsl:choose>
													<xsl:when test="fn:string(./xdf3:nameKey)">
														<xsl:value-of select="./xdf3:nameKey"/>
													</xsl:when>
													<xsl:otherwise>
												...
												<xs:restriction base="xs:token">
													<xsl:for-each-group select="$CodelisteInhalt/*/*:SimpleCodeList/*:Row" group-by="*:Value[@ColumnRef=$NameCodenameKey]">
														<xs:enumeration value="{./*:Value[@ColumnRef=$NameCodeKey]}">
															<xsl:if test="$Annotationen = '1'">
																<xs:annotation>
																	<xs:documentation>Auswahlwert: <xsl:value-of select="./*:Value[@ColumnRef=$NameCodenameKey]"/>
																	</xs:documentation>
																</xs:annotation>
															</xsl:if>
														</xs:enumeration>
						...
						<xsl:otherwise>
							...
							<xsl:if test="$Annotationen = '1'">
								<xs:annotation>
									<xs:documentation>Datenfeld: <xsl:value-of select="./xdf3:name"/> - <xsl:if test="$MitVersionen = '0' and fn:string(./xdf3:identifikation/xdf3:version)">Version: <xsl:value-of select="./xdf3:identifikation/xdf3:version"/> - </xsl:if>Typ: Auswahlliste (Keine Angabe einer Werte- oder Codeliste)</xs:documentation>
								</xs:annotation>
							</xsl:if>
						</xsl:otherwise>
					</xsl:choose>
				...
				<xsl:when test="./xdf3:datentyp/code/text() = 'text' or ./xdf3:datentyp/code/text() = 'text_latin'">
					<xsl:choose>
						<xsl:when test="$Praezisierungen = '1' and ./xdf3:praezisierung">
							<xsl:if test="$Annotationen = '1'">
								<xs:annotation>
									<xs:documentation>Datenfeld: <xsl:value-of select="./xdf3:name"/> - <xsl:if test="$MitVersionen = '0' and fn:string(./xdf3:identifikation/xdf3:version)">Version: <xsl:value-of select="./xdf3:identifikation/xdf3:version"/> - </xsl:if>Wertebereich: Text</xs:documentation>
								</xs:annotation>
							</xsl:if>
							<xs:simpleType>
				...
				<xsl:when test="./xdf3:datentyp/code/text() = 'date'">
					<xsl:choose>
						<xsl:when test="$Praezisierungen = '1' and ./xdf3:praezisierung">
							<xsl:if test="$Annotationen = '1'">
								<xs:annotation>
									<xs:documentation>Datenfeld: <xsl:value-of select="./xdf3:name"/> - <xsl:if test="$MitVersionen = '0' and fn:string(./xdf3:identifikation/xdf3:version)">Version: <xsl:value-of select="./xdf3:identifikation/xdf3:version"/> - </xsl:if>Wertebereich: Datum</xs:documentation>
								</xs:annotation>
							</xsl:if>
							<xs:simpleType>
				...
				<xsl:when test="./xdf3:datentyp/code/text() = 'time'">
					<xsl:choose>
						<xsl:when test="$Praezisierungen = '1' and ./xdf3:praezisierung">
							<xsl:if test="$Annotationen = '1'">
								<xs:annotation>
									<xs:documentation>Datenfeld: <xsl:value-of select="./xdf3:name"/> - <xsl:if test="$MitVersionen = '0' and fn:string(./xdf3:identifikation/xdf3:version)">Version: <xsl:value-of select="./xdf3:identifikation/xdf3:version"/> - </xsl:if>Wertebereich: Zeit</xs:documentation>
								</xs:annotation>
							</xsl:if>
							<xs:simpleType>
				...
				<xsl:when test="./xdf3:datentyp/code/text() = 'datetime'">
					<xsl:choose>
						<xsl:when test="$Praezisierungen = '1' and ./xdf3:praezisierung">
							<xsl:if test="$Annotationen = '1'">
								<xs:annotation>
									<xs:documentation>Datenfeld: <xsl:value-of select="./xdf3:name"/> - <xsl:if test="$MitVersionen = '0' and fn:string(./xdf3:identifikation/xdf3:version)">Version: <xsl:value-of select="./xdf3:identifikation/xdf3:version"/> - </xsl:if>Wertebereich: Zeitpunkt</xs:documentation>
								</xs:annotation>
							</xsl:if>
							<xs:simpleType>
				...
				<xsl:when test="./xdf3:datentyp/code/text() = 'bool'">
					...
					<xsl:if test="$Annotationen = '1'">
						<xs:annotation>
							<xs:documentation>Datenfeld: <xsl:value-of select="./xdf3:name"/> - <xsl:if test="$MitVersionen = '0' and fn:string(./xdf3:identifikation/xdf3:version)">Version: <xsl:value-of select="./xdf3:identifikation/xdf3:version"/> - </xsl:if>Wertebereich: Wahrheitswert</xs:documentation>
						</xs:annotation>
					</xsl:if>
				</xsl:when>
				<xsl:when test="./xdf3:datentyp/code/text() = 'num'">
					<xsl:choose>
						<xsl:when test="$Praezisierungen = '1' and ./xdf3:praezisierung">
							<xsl:if test="$Annotationen = '1'">
								<xs:annotation>
									<xs:documentation>Datenfeld: <xsl:value-of select="./xdf3:name"/> - <xsl:if test="$MitVersionen = '0' and fn:string(./xdf3:identifikation/xdf3:version)">Version: <xsl:value-of select="./xdf3:identifikation/xdf3:version"/> - </xsl:if>Wertebereich: Fließkommazahl</xs:documentation>
								</xs:annotation>
							</xsl:if>
							<xs:simpleType>
				...
				<xsl:when test="./xdf3:datentyp/code/text() = 'num_int'">
					<xsl:choose>
						<xsl:when test="$Praezisierungen = '1' and ./xdf3:praezisierung">
							<xsl:if test="$Annotationen = '1'">
								<xs:annotation>
									<xs:documentation>Datenfeld: <xsl:value-of select="./xdf3:name"/> - <xsl:if test="$MitVersionen = '0' and fn:string(./xdf3:identifikation/xdf3:version)">Version: <xsl:value-of select="./xdf3:identifikation/xdf3:version"/> - </xsl:if>Wertebereich: Ganzzahl</xs:documentation>
								</xs:annotation>
							</xsl:if>
							<xs:simpleType>
				...
				<xsl:when test="./xdf3:datentyp/code/text() = 'num_currency'">
					<xsl:choose>
						<xsl:when test="$Praezisierungen = '1' and ./xdf3:praezisierung">
							<xsl:if test="$Annotationen = '1'">
								<xs:annotation>
									<xs:documentation>Datenfeld: <xsl:value-of select="./xdf3:name"/> - <xsl:if test="$MitVersionen = '0' and fn:string(./xdf3:identifikation/xdf3:version)">Version: <xsl:value-of select="./xdf3:identifikation/xdf3:version"/> - </xsl:if>Wertebereich: Währungswert</xs:documentation>
								</xs:annotation>
							</xsl:if>
							<xs:simpleType>
				...
				<xsl:when test="./xdf3:datentyp/code/text() = 'file'">
					...
					<xsl:if test="$Annotationen = '1'">
						<xs:annotation>
							<xs:documentation>Datenfeld: <xsl:value-of select="./xdf3:name"/> - <xsl:if test="$MitVersionen = '0' and fn:string(./xdf3:identifikation/xdf3:version)">Version: <xsl:value-of select="./xdf3:identifikation/xdf3:version"/> - </xsl:if>Wertebereich: Anlage</xs:documentation>
						</xs:annotation>
					</xsl:if>
				</xsl:when>
				<xsl:when test="./xdf3:datentyp/code/text() = 'obj'">
					...
					<xsl:if test="$Annotationen = '1'">
						<xs:annotation>
							<xs:documentation>Datenfeld: <xsl:value-of select="./xdf3:name"/> - <xsl:if test="$MitVersionen = '0' and fn:string(./xdf3:identifikation/xdf3:version)">Version: <xsl:value-of select="./xdf3:identifikation/xdf3:version"/> - </xsl:if>Wertebereich: Statisches Objekt</xs:documentation>
						</xs:annotation>
					</xsl:if>
				</xsl:when>
				<xsl:otherwise>
					...
					<xsl:if test="$Annotationen = '1'">
						<xs:annotation>
							<xs:documentation>Datenfeld: <xsl:value-of select="./xdf3:name"/> - <xsl:if test="$MitVersionen = '0' and fn:string(./xdf3:identifikation/xdf3:version)">Version: <xsl:value-of select="./xdf3:identifikation/xdf3:version"/> - </xsl:if>Wertebereich: Unbekannt</xs:documentation>
						</xs:annotation>
					</xsl:if>
				</xsl:otherwise>
			</xsl:choose>
