...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:html="http://www.w3.org/1999/xhtml" xmlns="http://www.w3.org/1999/xhtml" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:xdf="urn:xoev-de:fim:standard:xdatenfelder_2" xmlns:gc="http://docs.oasis-open.org/codelist/ns/genericode/1.0/" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="html xs">
	...
	<xsl:variable name="StyleSheetName" select="'QS-DF_0_977_xdf2.xsl'"/> <!-- BackUp, falls fn:static-base-uri() leer -->
	...
	<xsl:template name="stammdatenschemadetails">
		...
		<tr>
			...
			<td>
				<xsl:choose>
					<xsl:when test="empty($Element/xdf:ableitungsmodifikationenStruktur/code/text())">
						<xsl:if test="$Meldungen = '1'">
							<xsl:call-template name="meldung">
		...
		<tr>
			...
			<td>
				<xsl:choose>
					<xsl:when test="empty($Element/xdf:ableitungsmodifikationenRepraesentation/code/text())">
						<xsl:if test="$Meldungen = '1'">
							<xsl:call-template name="meldung">
		...
		<tr>
			...
			<td>
				<xsl:choose>
					<xsl:when test="empty($Element/xdf:status/code/text())">
						<xsl:if test="$Meldungen = '1'">
							<xsl:call-template name="meldung">
					...
					<xsl:otherwise>
						...
						<xsl:if test="$Meldungen = '1' and $Element/xdf:status/code/text() != 'aktiv' and $VersionsHinweise ='1'">
							<xsl:call-template name="meldung">
	...
	<xsl:template name="datenfeldgruppedetails">
		...
		<tr>
			...
			<td>
				<xsl:choose>
					<xsl:when test="empty($Element/xdf:schemaelementart/code/text())">
						<xsl:if test="$Meldungen = '1'">
							<xsl:call-template name="meldung">
		...
		<xsl:choose>
			<xsl:when test="$Element/name() = 'xdf:datenfeld'">
				...
				<tr>
					...
					<td>
						<xsl:choose>
							...
							<xsl:otherwise>
								<xsl:apply-templates select="$Element/xdf:schemaelementart"/>
								<xsl:if test="$Meldungen = '1' and $Element/xdf:schemaelementart/code/text() = 'ABS' and $AbstraktWarnung ='1'">
									<xsl:call-template name="meldung">
				...
				<tr>
					...
					<td>
						<xsl:choose>
							<xsl:when test="empty($Element/xdf:feldart/code/text())">
								<xsl:if test="$Meldungen = '1'">
									<xsl:call-template name="meldung">
				...
				<tr>
					<td>Datentyp</td>
					<xsl:choose>
						<xsl:when test="empty($Element/xdf:datentyp/code)">
							<td>
								<xsl:if test="$Meldungen = '1'">
									<xsl:call-template name="meldung">
						...
						<xsl:otherwise>
							<td>
								...
								<xsl:if test="$Meldungen = '1'">
									<xsl:choose>
										<xsl:when test="$Element/xdf:feldart/code/text() = 'input'">
											<xsl:if test="$Element/xdf:datentyp/code/text() = 'obj'">
												<xsl:call-template name="meldung">
										...
										<xsl:when test="$Element/xdf:feldart/code/text() = 'select'">
											<xsl:if test="$Element/xdf:datentyp/code/text() != 'text'">
												<xsl:call-template name="meldung">
													<xsl:with-param name="nummer">1011</xsl:with-param>
												</xsl:call-template>
											</xsl:if>
										</xsl:when>
										<xsl:when test="$Element/xdf:feldart/code/text() = 'label'">
											<xsl:if test="$Element/xdf:datentyp/code/text() != 'text' and $Element/xdf:datentyp/code/text() != 'obj'">
												<xsl:call-template name="meldung">
					...
					<td>
						<xsl:if test="$Element/xdf:praezisierung/text() != ''">
							...
							<xsl:if test="substring-before(substring-after($Praezisierung,'pattern&quot;:&quot;'),'&quot;') != ''">
								...
								<xsl:if test="$Meldungen = '1' and ($Element/xdf:datentyp/code/text() = 'file' or $Element/xdf:datentyp/code/text() = 'obj')">
									<xsl:call-template name="meldung">
							...
							<td>
								...
								<xsl:if test="$Meldungen = '1'">
									...
									<xsl:choose>
										...
										<xsl:when test="$Element/xdf:datentyp/code/text() = 'file' or $Element/xdf:datentyp/code/text() = 'obj'">
											<xsl:if test="$Inhalt != ''">
												<xsl:call-template name="meldung">
